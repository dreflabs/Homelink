...
function FileTypeIcon({ type }: { type: string }) {
  if (type.startsWith("video/")) return <Film className="w-6 h-6 text-violet-500" strokeWidth={1.5} />;
  return <FileImage className="w-6 h-6 text-indigo-500" strokeWidth={1.5} />;
}
      ...
      >
        <CardContent className="flex flex-col items-center justify-center py-16 gap-4">
          <div className={`w-16 h-16 rounded-2xl flex items-center justify-center transition-colors ${isDragging ? "bg-indigo-100" : "bg-slate-100"}`}>
            <CloudUpload className={`w-8 h-8 transition-colors ${isDragging ? "text-indigo-600" : "text-slate-400"}`} strokeWidth={1.5} />
          </div>
