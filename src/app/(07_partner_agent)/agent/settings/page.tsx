...
import { Bell, Mail, MessageSquare, Smartphone, Globe, Moon, Shield } from "lucide-react";
...
const notificationSettings: SettingItem[] = [
  {
    id: "email_leads",
    label: "Notifikasi Email Leads Baru",
    description: "Terima email saat ada leads baru yang di-assign ke Anda",
    icon: Mail,
  },
  {
    id: "email_commission",
    label: "Update Status Komisi",
    description: "Notifikasi email saat status komisi berubah",
    icon: Shield,
  },
  {
    id: "push_reminder",
    label: "Pengingat Jadwal (Push)",
    description: "Notifikasi push 30 menit sebelum agenda",
    icon: Bell,
  },
  {
    id: "sms_closing",
    label: "SMS Notifikasi Closing",
    description: "Terima SMS saat transaksi berhasil ditutup",
    icon: Smartphone,
  },
  {
    id: "in_app",
...
const preferenceSettings: SettingItem[] = [
  {
    id: "dark_mode",
    label: "Mode Gelap",
    description: "Gunakan tampilan gelap untuk kenyamanan mata",
    icon: Moon,
  },
  {
    id: "lang_en",
    label: "Bahasa Inggris",
    description: "Tampilkan antarmuka dalam Bahasa Inggris",
    icon: Globe,
  },
];
    ...
    <div className="space-y-8 max-w-2xl">
      ...
      <div>
        ...
        <p className="text-slate-500 mt-1 text-sm">
          Kelola preferensi notifikasi dan tampilan akun Anda.
        </p>
      </div>
      ...
      <Card className="rounded-2xl shadow-sm border-slate-100 overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-100 flex items-center gap-2">
          ...
          <Badge variant="outline" className="text-xs ml-auto bg-blue-50 text-blue-700 border-blue-200">
            {Object.values(notifStates).filter(Boolean).length} aktif
          </Badge>
        </div>
      ...
      <Card className="rounded-2xl shadow-sm border-slate-100 p-6">
        ...
        <div className="space-y-3">
          ...
          <button className="w-full flex items-center justify-between px-4 py-3 rounded-xl bg-slate-50 hover:bg-slate-100 transition-colors text-sm">
            <span className="font-medium text-slate-700">Autentikasi Dua Faktor (2FA)</span>
            <Badge variant="outline" className="text-xs bg-amber-50 text-amber-700 border-amber-200">
              Belum Aktif
            </Badge>
          </button>
          <button className="w-full flex items-center justify-between px-4 py-3 rounded-xl bg-slate-50 hover:bg-slate-100 transition-colors text-sm">
            <span className="font-medium text-slate-700">Riwayat Login</span>
            <span className="text-slate-400">›</span>
          </button>
        </div>
      </Card>
