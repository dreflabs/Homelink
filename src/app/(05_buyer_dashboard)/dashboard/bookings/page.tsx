...
export default function BookingsPage() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case "CANCELLED":
        return <Badge className="bg-rose-100 text-rose-800 hover:bg-rose-100 border-rose-200">Dibatalkan</Badge>;
      default:
      ...
      <div className="flex flex-col md:flex-row">
        ...
        <div className="p-5 flex-1 flex flex-col justify-between bg-white">
          ...
          <div className="mt-5 pt-4 border-t border-gray-100 flex items-center justify-between">
            <Button variant="outline" className="text-gray-700 rounded-xl" onClick={() => alert('Fitur chat agen belum tersedia')}>
              Hubungi Agen
            </Button>
            ...
            {booking.status === "PENDING" && (
              <Button 
                ...
                onClick={() => {
                  if (confirm('Apakah Anda yakin ingin membatalkan jadwal survei ini?')) {
  ...
  return (
    <div className="max-w-4xl mx-auto pb-10">
      <div className="mb-8">
        <h1 className="text-3xl font-extrabold text-gray-900 tracking-tight">Jadwal Survei</h1>
        <p className="text-gray-500 mt-2">Kelola jadwal kunjungan survei properti Anda.</p>
      </div>
      ...
      <Tabs defaultValue="pending" className="w-full">
        <TabsList className="mb-6 p-1 bg-gray-100/80 rounded-xl inline-flex">
          <TabsTrigger value="pending" className="rounded-lg px-6 py-2.5 transition-all">
            Berjalan ({pendingBookings.length})
          </TabsTrigger>
        ...
        <TabsContent value="pending" className="mt-0 focus-visible:outline-none">
          {pendingBookings.length > 0 ? (
            ...
            <div className="text-center py-16 px-4 border border-dashed border-gray-200 rounded-2xl bg-gray-50/50">
              ...
              <h3 className="text-lg font-semibold text-gray-900 mb-1">Belum ada jadwal survei</h3>
              <p className="text-gray-500 max-w-sm mx-auto">Anda belum memiliki jadwal survei properti yang sedang berjalan saat ini.</p>
            </div>
          )}
        </TabsContent>
        ...
        <TabsContent value="history" className="mt-0 focus-visible:outline-none">
          {historyBookings.length > 0 ? (
            ...
            <div className="text-center py-16 px-4 border border-dashed border-gray-200 rounded-2xl bg-gray-50/50">
              ...
              <h3 className="text-lg font-semibold text-gray-900 mb-1">Riwayat kosong</h3>
              <p className="text-gray-500">Anda belum memiliki riwayat survei properti.</p>
            </div>
          )}
        </TabsContent>
      </Tabs>
